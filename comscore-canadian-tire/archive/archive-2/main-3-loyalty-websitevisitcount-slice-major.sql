...
only_visited_one_website_pet_intender_ids AS (
    SELECT guid
...
websites_visited AS (
    ...
    domain_intender_group = 'PETS' AND
    guid IN (
        SELECT guid FROM only_visited_one_website_pet_intender_ids
    )
    GROUP BY 1
)

SELECT * FROM websites_visited ORDER BY 2 DESC
