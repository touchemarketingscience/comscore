...
competitor_canadiantire AS (SELECT 
region,
...
SELECT 
    ...
    demo.machine_id
FROM 
    spectrum_comscore.person_demographics_ca AS demo 
LEFT JOIN 
    user_data_comscore AS data ON demo.machine_id = data.machine_id
WHERE 
    (date_part(year, data.calendar_date) >= 2021 AND date_part(year, data.calendar_date) <= 2022)
    */
LIMIT 10000;
