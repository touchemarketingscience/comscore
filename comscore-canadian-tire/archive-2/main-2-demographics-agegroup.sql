...
total_intenders AS (
    ...
    WHERE date_part(year, date) = 2021 OR date_part(year, date) = 2022
    AND machine_id IN (
        SELECT machine_id FROM pet_intender_machine_ids
    )
    GROUP BY person_id
),

total_comscore_age AS (
    SELECT
    (CASE 
...
FROM total_comscore_age AS a LEFT JOIN total_intenders_age AS b
