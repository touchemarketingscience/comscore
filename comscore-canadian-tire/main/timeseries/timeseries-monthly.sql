WITH intenders AS (
    ...
    WHERE 
    (
        date_part(year, calendar_date) = 2019 OR date_part(year, calendar_date) = 2022
    ) 
    AND 
    (   
        -- ********************************************************************
...
audience AS (
    SELECT
    CONCAT(date_part(year, calendar_date), '-', date_part(month, calendar_date)) AS month_year,
    COUNT(DISTINCT guid) as unique_users
    FROM spectrum_comscore.clickstream_ca
    WHERE 
    (
        date_part(year, calendar_date) = 2019 OR date_part(year, calendar_date) = 2020 OR date_part(year, calendar_date) = 2021 OR date_part(year, calendar_date) = 2022
    )
    AND 
    (
        guid IN 
