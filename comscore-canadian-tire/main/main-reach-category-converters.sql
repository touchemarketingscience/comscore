...
pet_converters AS (SELECT 
    ...
    FROM user_data_comscore WHERE guid IN (
        ...
        FROM user_data_comscore
        WHERE domain_intender_group = 'PETS' AND is_converter = 'CONVERTER'
    )
    GROUP BY 1
)
...
SELECT
    a.primary_field,
    a.unique_users as unique_users_pet_converters,
    b.unique_users as unique_users_comscore
FROM pet_converters AS a LEFT JOIN comscore_total AS b 
