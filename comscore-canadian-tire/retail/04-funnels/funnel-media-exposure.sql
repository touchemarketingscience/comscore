...
total_adconsumers AS (
    SELECT
        domain_group AS join_field_a,
        COUNT(DISTINCT guid) AS unique_users
    FROM unique_adconsumer_data
    WHERE ((calendar_date) >= (SELECT value FROM date_lower_bound) AND calendar_date <= (SELECT value FROM date_upper_bound))
    GROUP BY 1
),
...
total_output AS (
    SELECT 
        ...
        total_adconsumers.unique_users  AS total_adconsumers,
    ...
    LEFT JOIN total_adconsumers     ON total_intenders.join_field_a = total_adconsumers.join_field_a
),
...
SELECT
    ...
    ref_adconsumers.unique_users        AS ref_adconsumers,
...
CROSS JOIN  ref_adconsumers


-- COMPETITIVE SET EXCLUSION+VOLUME FILTER
-- WHERE total_locators > 50 AND NOT 
-- ( -- INTENDER LOGIC
