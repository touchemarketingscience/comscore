...
total_website_visit_count_per_converter_1ONLY AS (
    SELECT
        guid,
...
total_website_visit_count_per_converter_2PLUS AS (
    SELECT
        guid,
        COUNT(DISTINCT domain_group) AS domain_group_count
    FROM unique_converter_data
...
total_output_1ONLY AS (
    ...
    FROM unique_converter_data WHERE guid IN (
        SELECT guid FROM total_website_visit_count_per_converter_1ONLY
    )
    GROUP BY 1
),
...
total_output_2ONLY AS (
    ...
    COUNT(DISTINCT guid) AS unique_users
    FROM unique_converter_data WHERE guid IN (
        SELECT guid FROM total_website_visit_count_per_converter_2PLUS
    )
    GROUP BY 1
),
...
b.unique_users AS ref_converters,
c.unique_users AS ref_genpop
FROM total_output AS a
CROSS JOIN ref_converters AS b
