...
def process_words(words, frequency, keywords):
    words = [word.replace('.html', '').replace(',', '').lower() for word in words 
             if len(word) > 2 
    ...
    for word in words:
        keywords.extend([word]*frequency)
...
for _, row in tqdm(df.iterrows(), total=df.shape[0], desc="Processing URLs"):
    ...
    if split_key:
        ...
        if len(path_list) > 1:  # If split_key is found in path, the length of path_list will be more than 1
            ...
            process_words(words, frequency, keywords)
    else:
...
word_cloud = WordCloud(background_color='white', colormap='Accent', width = 1597, height = 610).generate_from_frequencies(word_counts)
