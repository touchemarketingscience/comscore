...
# remove any rows where 'url' is not a string
df = df[df['url'].apply(lambda x: isinstance(x, str))]

# Convert 'frequency' to integer
df['frequency'] = df['frequency'].apply(lambda x: int(x) if pd.notnull(x) else 0)
...
for _, row in tqdm(df.iterrows(), total=df.shape[0], desc="Processing URLs"):
    ...
    if 'homedepot.ca' in path:
        split_key = 'homedepot.ca'
    ...
    if split_key:
        ...
        if len(path_list) > 1:  # If split_key is found in path, the length of path_list will be more than 1
            ...
            words = [word for word in words if not word.isdigit() and word.lower() not in exclusion_list and '%' not in word and 'cmd' not in word.lower() and 'homedepot' not in word.lower() and 'view' not in word.lower() and 'detail' not in word.lower() and'.' not in word and 'search' not in word.lower()]
            for word in words:
...
word_cloud = WordCloud(background_color='white', colormap='Oranges', width = 1597, height = 610).generate_from_frequencies(word_counts)
